...
/**
 * Structure of a single TripFareResult. This may cover the whole trip or only the part the service can answer. If only partial answers can be delivered, then this must be indicated with
 * FARE_ADDITIONALTICKETS (as a warning). For a given trip multiple FareResults may need to be combined to get the whole tariff. A typical example may be classic public transport and a sharing leg.
 * Some legs, e.g., TransferLegs and walking ContinuousLeg will never have a price. The service may include them in one of the tickets. If the whole trip is for free, then a TripFareResult with a
 * price of 0 is expected. It is possible to have combined FareResults (e.g., for a bundle) and in addition price each leg individually.
 *
...
public class TripFareResultStructure {
    ...
    /**
     * A ticket may be valid for multiple non-consecutive legs. TransferLegs and ContinuousLegs without a tariff may be either included or ignored. If the tariff depends on not having a ContinuousLeg
     * e.g., with sharing in the middle, then the server must decide, if it wants to provide more than one TripFareResult.Gets the value of the legRangeGroup property.
     *
     * <p>
     * This accessor method returns a reference to the live list, not a snapshot. Therefore any modification you make to the returned list will be present inside the Jakarta XML Binding object. This
     * is why there is not a <CODE>set</CODE> method for the legRangeGroup property.
     *
     * <p>
     * For example, to add a new item, do as follows:
     * <pre>
